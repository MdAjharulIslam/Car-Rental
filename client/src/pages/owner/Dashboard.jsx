...
const fetchDashboardData = async () => {
  ...
  <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 my-8 max-w-7xl">
    {dashboardCards.map((card, index) => (
      <div
        key={index}
        className="flex items-center justify-between gap-4 bg-white rounded-xl shadow-sm border border-borderColor p-4 transition hover:shadow-md"
      >
        <div>
          <h1 className="text-sm text-muted-foreground">{card.title}</h1>
  ...
  <div className='flex flex-wrap items-start gap-6 mb-8 w-full'>
    ...
    <div className='p-4 md:p-6 mb-6 border border-borderColor rounded-md w-full md:max-w-xs'>
      ...
      <p className='text-gay-500'>Revenue for current month</p>
